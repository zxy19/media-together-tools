...
<?php
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://api.xingzhige.com/API/QQmusicVIP/?name=".urlencode($_GET['word']));
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://api.xingzhige.com/API/QQmusicVIP/?name=".urlencode($_GET['word'])."&n=".$_GET['n']);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); 
    curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36');
    $response = curl_exec($ch);
    if(curl_errno($ch)) {
